...
EOF
docker run --name ftp --rm -d --env FTP_USER_UID=$(id -u) --env FTP_USER_GID=$(id -g) -p 8021:21 -p 30000-30009:30000-30009 -v `pwd`/_scratch/ftp-test/:/home/ftpusers -v `pwd`/_scratch/ftp-var:/var/ftp -v `pwd`/_scratch/passwd:/etc/pure-ftpd/passwd ohsucompbio/funnel-ftp-test-server
