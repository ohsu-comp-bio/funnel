...
func (s *svc) CreateInstanceProfile(ctx context.Context) (string, error) {
	iamCli := iam.New(s.sess)

	resp, err := iamCli.GetInstanceProfileWithContext(ctx, &iam.GetInstanceProfileInput{
		InstanceProfileName: aws.String(s.conf.IamInstanceProfile.Name),
	})
	if err == nil {
		for _, r := range resp.InstanceProfile.Roles {
			if *r.RoleName == s.conf.IamInstanceProfile.Name {
				return *resp.InstanceProfile.Arn, awsutil.ErrResourceExists{}
			}
		}
	}
	...
	if err != nil {
		return "", fmt.Errorf("error marshaling AssumeRole policy")
	}

	_, err = iamCli.CreateRoleWithContext(ctx, &iam.CreateRoleInput{
		AssumeRolePolicyDocument: aws.String(string(roleb)),
		RoleName:                 aws.String(s.conf.IamInstanceProfile.Name),
	})
	if err != nil {
		return "", err
	}

	cip, err := iamCli.CreateInstanceProfileWithContext(ctx, &iam.CreateInstanceProfileInput{
		InstanceProfileName: aws.String(s.conf.IamInstanceProfile.Name),
	})
	if err != nil {
		return "", err
	...
	_, err = iamCli.AddRoleToInstanceProfileWithContext(ctx, &iam.AddRoleToInstanceProfileInput{
		InstanceProfileName: aws.String(s.conf.IamInstanceProfile.Name),
		RoleName:            aws.String(s.conf.IamInstanceProfile.Name),
	})
	if err != nil {
	...
	return *cip.InstanceProfile.Arn, nil
}
...
func (s *svc) AttachRolePolicies(ctx context.Context) error {
	...
	if len(resp.PolicyNames) > 0 {
		...
		if strings.Contains(policies, "FunnelEBSDynamoDB") &&
			strings.Contains(policies, "FunnelEBSBatch") &&
			strings.Contains(policies, "AWSElasticBeanstalkWebTier") &&
			strings.Contains(policies, "AWSElasticBeanstalkMulticontainerDocker") &&
			strings.Contains(policies, "AWSElasticBeanstalkWorkerTier") {
			return awsutil.ErrResourceExists{}
		}
	}

	// attach the appropriate managed policies provided by Elastic Beanstalk
	_, err = iamCli.AttachRolePolicy(&iam.AttachRolePolicyInput{
		PolicyArn: aws.String("arn:aws:iam::aws:policy/AWSElasticBeanstalkWebTier"),
		RoleName:  aws.String(s.conf.IamInstanceProfile.Name),
	})
	if err != nil {
		return fmt.Errorf("error attaching AWSElasticBeanstalkWebTier policy")
	}
	_, err = iamCli.AttachRolePolicy(&iam.AttachRolePolicyInput{
		PolicyArn: aws.String("arn:aws:iam::aws:policy/AWSElasticBeanstalkMulticontainerDocker"),
		RoleName:  aws.String(s.conf.IamInstanceProfile.Name),
	})
	if err != nil {
		return fmt.Errorf("error attaching AWSElasticBeanstalkMulticontainerDocker policy")
	}
	_, err = iamCli.AttachRolePolicy(&iam.AttachRolePolicyInput{
		PolicyArn: aws.String("arn:aws:iam::aws:policy/AWSElasticBeanstalkWorkerTier"),
		RoleName:  aws.String(s.conf.IamInstanceProfile.Name),
	})
	if err != nil {
		return fmt.Errorf("error attaching AWSElasticBeanstalkWorkerTier policy")
	}
	...
	dynamob, err := json.Marshal(s.conf.IamInstanceProfile.Policies.DynamoDB)
	if err != nil {
...
func (s *svc) CreateEnvironment(ctx context.Context) (*ebs.EnvironmentDescription, error) {
	...
	if resp.Environments != nil {
		if len(resp.Environments) > 0 {
			switch *resp.Environments[0].Status {
	...
	instanceRole, err := s.CreateInstanceProfile(ctx)
	if err != nil {
